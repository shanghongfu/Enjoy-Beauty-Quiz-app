...
function parseQuestionsFromText(text) {
  ...
  while (i < lines.length) {
    ...
    if (qMatch || isStemByQuestion) {
      ...
      i++;
      // Bilingual stems: merge CJK translation line(s) following the (English) stem.
      // Handles same-line EN+ZH (no merge needed), split lines, blank-line spacing,
      // and Word auto-numbered ZH lines (e.g. "2.中文..."). Stops at options/answers/new questions.
      // Only merges when the stem itself has no CJK, so a Chinese stem is never polluted by its options.
      while (i < lines.length) {
        const nxt = lines[i];
        const isAns = isAnswerLine(nxt) || /^(解析|explanation|analysis|说明|rationale)\s*[:：.\s]/i.test(nxt);
        if (isAns) break;
        const nxtHasCJK = /[\u4e00-\u9fff]/.test(nxt);
        const isOptLetter = /^[A-Za-z][.、)）\s]/.test(nxt);
        const isNewQ = /^(\d+)[.、)）\s]|^第?\s*\d+\s*[题个]|^Q\s*\d+/i.test(nxt);
        if (nxtHasCJK && !isOptLetter && !/[\u4e00-\u9fff]/.test(stem)) {
          let t = nxt.replace(/^\d+[.、)）\s]+/, '').trim();   // strip Word auto-number prefix like "3."
          stem = (stem + ' ' + t).trim();
          i++;
          continue;
        }
        if (isOptLetter || isNewQ) break;
        break;
      }
      q.stem = stem;  // 写回合并后的题干（双语导入时 stem 已被追加中文翻译）
